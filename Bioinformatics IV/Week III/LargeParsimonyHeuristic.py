...
def SmallParsUnrooted_wrapper(adj_list):
...
def printTree(min_pars_score, final_adj_list):
...
def NearestNeighborInterchange(adj_list):

    org_list = deepcopy(adj_list)
    strings = [x[0] for x in adj_list if not RepresentsInt(x[0])]
    score = 1e6
    newTree, newScore = SmallParsUnrooted_wrapper(adj_list)
    newTree = org_list
...
if __name__ == "__main__":
    ...
    NearestNeighborInterchange(adj_list)