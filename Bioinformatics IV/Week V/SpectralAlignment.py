...
def SpectralAlignment(peptide, spectral_vector, k):
        ...
        for j in range(len(spectral_vector)):
            for t in range(k + 1):
                if (t == 0) and (i - diff[i] >= 0) and (j - diff[i] >= 0):
                    ...
                    Score[i][j][t] = spectral_vector[j] + max(Score[i - diff[i]][j - diff[i]][t], max([Score[i - diff[i]][j_star][t - 1] for j_star in range(j)]))
                elif (t > 0) and (i - diff[i] >= 0) and (j > 0):
                    Score[i][j][t] = spectral_vector[j] + max([Score[i - diff[i]][j_star][t - 1] for j_star in range(j)])
    ...
    for i in range(len(peptide), 0, -1):
        pre = prefixMasses[i]
        if (column - diff[pre] >= 0) and (Score[pre][column][layer] == spectral_vector[column] + Score[pre - diff[pre]][column - diff[pre]][layer]):
            column -= diff[pre]
