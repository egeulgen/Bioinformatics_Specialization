...
def LinearSpaceAlignment(v, w, top, bottom, left, right):
    ...
    if bottom == top:
        return ('H' * (right - left))
    mid_edge, mid_from, mid_to = MiddleEdge(v, w, top, bottom, left, right)
    ## Left
    midNode, middle = mid_from
    pathL = LinearSpaceAlignment(v, w, top, midNode, left, middle)
    ## Right
    midNode, middle = mid_to
    pathR = LinearSpaceAlignment(v, w, midNode, bottom, middle, right)
    return pathL + mid_edge + pathR

def TrackPath(path, v, w):
    res_v = ''
    res_w = ''
    i = 0
    j = 0
...
if __name__ == "__main__":
    ...
    res_v, res_w = TrackPath(path, v, w)
